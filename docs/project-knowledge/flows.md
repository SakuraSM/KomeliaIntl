...
Android and desktop comic import and reading share `ComicArchiveService` through the JVM shared source set. Filename extensions discover candidates, including 7z and CB7; signatures select ZIP, RAR or 7z. A renamed archive retains its real media type and source identity. EPUB remains ZIP-only and keeps its publication validation; PDF uses its existing platform path.

`AndroidArchiveSourceAccess` provides direct seekable access or a bounded temporary copy for non-seekable providers. EPUB's `AndroidZipArchiveOpener` uses the same access layer. `ArchiveScratchSpace` counts provider copies and 7z caches together: 1 GiB per file, 2 GiB total, with 128 MiB of storage reserved. Permission and parser failures do not trigger alternative parsers.

`SevenZipCacheConverter` sequentially validates and converts 7z to a private, uncompressed ZIP. It limits each decoded entry to 64 MiB, the total to 1 GiB, entries to 10,000, and decoder memory to 128 MiB. A process-wide lock permits one conversion at a time. It rejects unsafe or duplicate paths, encrypted entries and unsupported methods. A checked `.part` file becomes a cache only through atomic rename. Original archives are never modified or extracted into user folders.

The service keeps at most two archives across live handles and inactive disk caches. Cache keys include source identity, size, modification time and a format version. Missing modification times add a process-session key. Source changes, failure, cancellation and eviction retire owned resources. Completed caches survive restart; startup removes only owned temporary or obsolete cache files. Capacity failures can reclaim idle caches and retry the same operation once. The local-folder and reader UIs share localized failure messages and observe transient preparation state. Manual scans have an explicit Cancel action; scan failures preserve existing book records and progress.
