...
- Image/PDF reading hides both Android status and navigation bars while reader controls are hidden. Showing controls and leaving the reader restore the bars. EPUB keeps its separate immersion setting. Hidden navigation uses `BEHAVIOR_DEFAULT` so edge Back remains available. Verify real edge Back in gesture mode, reveal and use Back in three-button mode, PDF two-step Back, tablet taskbar visibility, and restoration on exit.
